...
def jetSelection(event, ptcut=20.0, etacut=2.4):
    def jetfilter(jet):
        return jet.PT > ptcut and abs(jet.Eta) < etacut

    return list(filter(jetfilter, event.jets))
...
def bjets(event):
    ...
    return list(filter(jetfilter, event.selectedJets))
