...
def DumpEventInfo(event=None, eventNumber=None, path="./"):
    ...
    for product in eventDumpConfig.productsToPrint:
        getattr(event, product)
    for collection in eventDumpConfig.collectionsToHide:
        event.removeCollection(collection)
