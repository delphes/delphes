...
parser.add_option("-c", "--conf", dest="conf", default=None, help="Configuration file.")
parser.add_option("-i", "--inputPath", dest="path", help="Read input file from DIR.", metavar="DIR")
parser.add_option("-o", "--output", dest="outputname", default=None, help="Save output as FILE.", metavar="FILE")
parser.add_option("--all", action="store_true", dest="all", help="Process all levels.")
parser.add_option("-l", "--level", dest="levels", help="Specify a coma-separated list of levels to be processed. No space is allowed.")
parser.add_option("--Njobs", type="int", dest="Njobs", default="1", help="Number of jobs when splitting the processing.")
parser.add_option("--jobNumber", type="int", dest="jobNumber", default="0", help="Number of the job is a splitted set of jobs.")
...
def main(options):
    ...
    levels = []
    if options.all:
        levels = list(range(EventSelection.eventCategories()))
    elif options.levels is not None:
        levels = list(map(int, options.levels.split(",")))
    levels.sort()
    if len(levels) == 0:
...
def runAnalysis(path, levels, outputname="controlPlots.root", Njobs=1, jobNumber=1):
    ...
    for event in events:
        ...
        if configuration.runningMode == "plots":
            # loop on channels
            plots = [x for x in levels if EventSelection.isInCategory(x, event.category)]
            ...
            for cp in controlPlots[:1]:
                ...
                cp.setCategories([EventSelection.isInCategory(c, event.category) for c in range(EventSelection.eventCategories())])
...
def createDirectory(dirStructure, directory, leafList):
    ...
    for key, item in list(dirStructure.items()):
