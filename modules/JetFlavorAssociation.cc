...
void JetFlavorAssociation::Init()
{
  ...
  if(fParticleLHEFInputArray)
    fParticleLHEFFilter = new ExRootSTLVectorFilter(*fParticleLHEFInputArray);
}
...
void JetFlavorAssociation::Process()
{
  ...
  fPartonFilter->Reset();
  const auto partonArray = fPartonFilter->GetSubArray(fPartonClassifier, 0); // get the filtered parton array
  ...
  for(const auto &parton : partonArray)
  {
    ...
    for(const auto &partonLHEF : partonLHEFArray)
    {
      ...
      if(parton.D1 != -1 || parton.D2 != -1)
      {
        ...
        if(parton.D1 != -1) daughterFlavor1 = TMath::Abs(fParticleInputArray->at(parton.D1).PID);
        if(parton.D2 != -1) daughterFlavor2 = TMath::Abs(fParticleInputArray->at(parton.D2).PID);
...
void JetFlavorAssociation::GetPhysicsFlavor(Candidate &jet, const std::vector<Candidate> &partonArray, const std::vector<Candidate> &partonLHEFArray)
{
  ...
  vector<const Candidate *> contaminations;
  ...
  else if(contaminations.size() > 0)
  {
    ...
    for(const auto *parton : contaminations)
    {
