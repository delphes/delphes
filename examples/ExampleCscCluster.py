    ...
    for entry in range(0, numberOfEntries):
        ...
        for i in range(branchElectron.GetEntries()):
            if branchElectron.At(i).PT > 35 and abs(branchElectron.At(i).Eta) < 2.5:
                nLeptons += 1
        for i in range(branchElectron.GetEntries()):
            if branchElectron.At(i).PT > 25 and abs(branchElectron.At(i).Eta) < 2.4:
                nLeptons += 1
        if nLeptons > 0:
            continue
        ################################
        ...
        for i in range(branchCluster.GetEntries()):
            ...
            nCscCluster += 1
            if maxJetVetoPt < 10 and abs(dPhi(cluster.Phi, branchMET.At(0).Phi)) < 0.75 and cluster.T < 12.5 and cluster.T > -5:
                nCscCluster += 1
        if nCscCluster == 0:
            continue
